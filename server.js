...
// ─────────────────────────────────────────────
//  PHP FETCHER
//  Strips any HTML the hosting injects, extracts JSON
// ─────────────────────────────────────────────
async function phpFetch(params) {
  ...
  console.log('[phpFetch] calling:', url.toString());

  const res = await fetch(url.toString(), {
    signal: AbortSignal.timeout(15000),
    headers: { 'User-Agent': 'SMSBridge/3.0', 'Accept': 'application/json, text/plain, */*' }
  });

  const raw = await res.text();
  console.log('[phpFetch] raw (first 200):', raw.slice(0, 200));

  // Find JSON start — strips any HTML the host injects
  const start = raw.search(/[{\[]/);
  if (start === -1) throw new Error('No JSON in response: ' + raw.slice(0, 150));
  const end = Math.max(raw.lastIndexOf('}'), raw.lastIndexOf(']')) + 1;
  const parsed = JSON.parse(raw.slice(start, end));
  console.log('[phpFetch] parsed ok, status:', parsed.status);
  return parsed;
}
...
async function syncFromPHP() {
  try {
    ...
    console.log(`[sync] SUCCESS — pending:${store.pending.length} sent:${store.sent.length} failed:${store.failed.length}`);
    return true;
  } catch (e) {
...
// ─────────────────────────────────────────────
//  PING — also shows last sync status
// ─────────────────────────────────────────────
...
app.all('/sms_gateway.php', async (req, res) => {
  ...
  // PENDING — always sync fresh from PHP first
  if (action === 'pending') {
    await syncFromPHP();
...
app.listen(PORT, async () => {
  ...
  // retry sync on startup up to 3 times
  for (let i = 1; i <= 3; i++) {
