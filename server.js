...
// Point to the NEW clean bridge file
const PHP_API_URL = process.env.PHP_API_URL || 'http://lld.zf3r.com/sms_bridge.php';
...
// ─────────────────────────────────────────────
//  PHP FETCHER — clean JSON expected from sms_bridge.php
// ─────────────────────────────────────────────
async function phpFetch(params) {
  ...
  const res = await fetch(url.toString(), {
    ...
    headers: { 'User-Agent': 'SMSBridge/4.0', 'Accept': 'application/json' }
  });

  const raw = await res.text();
  console.log('[phpFetch] raw:', raw.slice(0, 200));

  // Strip any BOM or stray chars before first { or [
...
async function syncFromPHP() {
  try {
    ...
    // sms_bridge.php returns messages[] with {id, to, text, created_at}
    store.pending = (pd.messages || []).map(m => ({
      id      : parseInt(m.id),
      phone_to: m.to   || m.phone   || '',
...
app.listen(PORT, async () => {
  ...
  console.log(`PHP Bridge: ${PHP_API_URL}`);
  for (let i = 1; i <= 3; i++) {
